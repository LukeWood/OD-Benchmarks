...
TRAIN_DATA = 'gs://practical-ml-vision-book/arthropod_detection_tfr/size_w1024px/*.train.tfrec'
VALID_DATA = 'gs://practical-ml-vision-book/arthropod_detection_tfr/size_w1024px/*.test.tfrec'
...
features = tfds.features.FeaturesDict({
    'image/encoded':   tfds.features.Image(shape=(None, None, 3), encoding_format='jpeg'), # automatic JPEG compression/decompresion
    ...
    'image/object/class/label': tfds.features.Sequence(tfds.features.Tensor(shape=(), dtype=tf.int64)),
    'image/object/bbox/xmin':   tfds.features.Sequence(tfds.features.Tensor(shape=(), dtype=tf.float32)),
    'image/object/bbox/ymin':   tfds.features.Sequence(tfds.features.Tensor(shape=(), dtype=tf.float32)),
    'image/object/bbox/xmax':   tfds.features.Sequence(tfds.features.Tensor(shape=(), dtype=tf.float32)),
    'image/object/bbox/ymax':   tfds.features.Sequence(tfds.features.Tensor(shape=(), dtype=tf.float32)),
})
...
def unpackage_raw_inputs(inputs, bounding_box_format):
    ...
    boxes = tf.stack([data['image/object/bbox/xmin'],
                      data['image/object/bbox/ymin'],
                      data['image/object/bbox/xmax'],
                      data['image/object/bbox/ymax']], axis=-1)
...
def load(split, bounding_box_format):
   ...
   # list files and decode TFRecords
    ...
    ignore_order = tf.data.Options() # Order does not matter since we will be shuffling the data anyway.
    ...
    # process the data
    dataset = dataset.map(lambda x: unpackage_raw_inputs(x, bounding_box_format), num_parallel_calls=AUTO)
    dataset = dataset.map(lambda x: format_inputs(x, bounding_box_format), num_parallel_calls=AUTO)
